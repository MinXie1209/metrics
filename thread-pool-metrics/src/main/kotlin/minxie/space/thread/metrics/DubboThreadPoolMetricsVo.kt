        ...
        ThreadPoolContext.getDubboThreadPoolMap().filter { it.key is ThreadPoolExecutor }
            .forEach { (it, threadPoolName) ->
                ...
                it.activeCount.let { activeCount ->
                    MetricItemVo.build(MetricKeyEnum.DUBBO_THREAD_POOL_ACTIVE_COUNT, activeCount.toFloat(), tag)
                        ?.let { metricItem -> metricList.add(metricItem) }
                }

                it.corePoolSize.let { corePoolSize ->
                    MetricItemVo.build(MetricKeyEnum.DUBBO_THREAD_POOL_CORE_POOL_SIZE, corePoolSize.toFloat(), tag)
                        ?.let { metricItem -> metricList.add(metricItem) }
                }

                it.maximumPoolSize.let { maximumPoolSize ->
                    MetricItemVo.build(
                        MetricKeyEnum.DUBBO_THREAD_POOL_MAXIMUM_POOL_SIZE,
                ...
                it.queue?.let { queue ->
                    MetricItemVo.build(MetricKeyEnum.DUBBO_THREAD_POOL_QUEUE_SIZE, queue.size.toFloat(), tag)
                        ?.let { metricItem -> metricList.add(metricItem) }
                }

                it.completedTaskCount.let { completedTaskCount ->
                    MetricItemVo.build(
                        MetricKeyEnum.DUBBO_THREAD_POOL_COMPLETED_TASK_COUNT,
