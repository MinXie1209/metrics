...
class DubboThreadPoolMetricsVo : MetricBaseVo() {
    init {
        ThreadPoolContext.getDubboThreadPoolSet().filterIsInstance<ThreadPoolExecutor>().forEach {
            ...
            it.completedTaskCount?.let { completedTaskCount ->
                metricList.add(MetricItemVo(DUBBO_THREAD_POOL_COMPLETED_TASK_COUNT, completedTaskCount.toFloat(), tag))
            }
